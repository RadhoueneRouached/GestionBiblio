...
<body>
	...
	<div id="conteneur">
		...
		<div id="center">
			...
			<!-- CONTENU  -->
			<div id="content">
				<form id="formID" class="formular" action="${pageContext.request.contextPath}/edit/${adherent.id_adherent}" modelAttribute="adherent"  modelAttribute="adresse"
					method="post">
					<fieldset>
						...
						<label> <input
							...
							type="hidden" name="idAdherent" value="${adherent.id_adherent}" id="idAdherent"  /> <label>
								<span><fmt:message key='lastName' /></span> <input
								class="validate[required,custom[noSpecialCaracters],length[1,20]]"
								type="text" name="nom" value="${adherent.nom}" id="nom" />
						</label> <label> <span><fmt:message key='firstName' /></span> <input
								class="validate[required,custom[noSpecialCaracters],length[1,20]]"
								type="text" name="prenom" id="prenom" value="${adherent.prenom}" />
						</label> <label> <span><fmt:message key='nic' /></span> <input
								class="validate[required,custom[onlyNumber],length[1,20]]"
								type="text" name="cin" value="${adherent.cin}" id="cin" /> <label> <span><fmt:message key='warning' /></span>
									<input
									class="validate[required,custom[onlyNumber],length[1,20]]"
									type="text" name="avertissement" value="${adherent.active}" id="avertissement" />

							</label> <label> <span><fmt:message key='birthday' /></span> <input
									class="validate[required,length[1,20]]"
									type="text" name="DateNaissance" id="DateNaissance" value="${adherent.dateNaissance}"/>
							</label> <label> <span><fmt:message key='Email' /></span> <input
									class="validate[required,custom[email],length[1,20]]"
									type="text" name="email" id="email" value="${adherent.email}" />

							</label> <label> <span><fmt:message key='pass' /></span> <input
									class="validate[required,confirm[password],length[1,20]]"
									type="text" name="Password" id="password" value="${adherent.pass}" />
							</label> <label> <span><fmt:message key='roadNumber' /></span> <input
									class="validate[required,custom[onlyNumber],length[1,20]]"
									type="text" name="nrue" value="${adherent.getAdresse().numRue}" id="nrue" />
							</label> <label> <span><fmt:message key='road' /></span> <input
									class="validate[required,custom[noSpecialCaracters],length[1,20]]"
									type="text" name="rue" id="rue" value="${adherent.getAdresse().rue}" />
							</label> <label> <span><fmt:message key='city' /></span> <input
									class="validate[required,custom[noSpecialCaracters],length[1,20]]"
									type="text" name="cite" value="${adherent.getAdresse().cite}" id="cite" />
							</label> <label> <span><fmt:message key='state' /></span> <input
									class="validate[required,custom[noSpecialCaracters],length[1,20]]"
									type="text" name="ville"  value="${adherent.getAdresse().ville}" id="ville" />
							</label>
					</fieldset>
					<p>
